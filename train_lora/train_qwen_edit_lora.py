...
import gc
import math
from qwen_image_edit_plus_pipeline import QwenImageEditPlusPipelineWithStyleControl
from qwen_image_edit_plus_processor import QwenDoubleStreamAttnProcessor2_0WithStyleControl
from qwen_image_edit_plus_transformer_qwenimage import QwenImageTransformer2DModel
    ...
    for epoch in range(10000000):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                # packed_control_img = QwenImageQwenImageEditPlusPipelineWithStyleControlEditPipeline._pack_latents(
                #     control_img,
                #     bsz, 
                #     control_img.shape[2],
                #     control_img.shape[3],
                ...
                print("packed_noisy_model_input_concated shape:", packed_noisy_model_input_concated.shape)
                with torch.no_grad():
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    # [DeepSpeed Zero3 Fix] 使用 get_state_dict 从所有 GPU 收集完整权重
                    # 注意：这会将完整权重加载到 CPU 内存中，确保内存足够
                    full_model_state_dict = accelerator.get_state_dict(flux_transformer)
                    
                    # 获取 unwrapped 模型结构 (此时权重可能是空的或分片的，主要为了获取结构)
                    unwrapped_flux_transformer = unwrap_model(flux_transformer)
                    
                    # 将收集到的完整 state_dict 传给 PEFT 工具，显式提取 LoRA 权重
                    # 只有主进程会执行保存，但 get_state_dict 内部会有同步操作
                    flux_transformer_lora_state_dict = convert_state_dict_to_diffusers(
                        get_peft_model_state_dict(unwrapped_flux_transformer, state_dict=full_model_state_dict)
                    )
