    ...
    def __call__(
        ...
        **kwargs, # 通过 kwargs 捕获 joint_attention_kwargs
        ...
        if latent_mask is not None and seq_lengths is not None:
            ...
            # --- v9: 动态逻辑切换 ---
            # is_early_step = current_step_fraction < switch_point_fraction
            # use_layout_logic = (is_early_step and not reverse_logic) or (not is_early_step and reverse_logic)
            
            # 简化：
            # 正常: 早期 (True) < 0.5 (True) -> (True and True) = True
            # 反转: 早期 (True) < 0.5 (True) -> (True and False) = False
            use_layout_logic = (current_step_fraction < switch_point_fraction) ^ reverse_logic
