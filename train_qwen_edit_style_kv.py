...
def main():
    ...
    #del vae
    gc.collect()
    print('Finished caching embeddings.')
    ...
    # 3. 【核心修改】替换 Attention Processor
    # 假设 style_context_dim 为 64 (根据你的注释 16*4)
    style_context_dim = 64 
    attn_processors = {}
    
    # 遍历现有的 processor 并创建新的 processor 实例
    for name in flux_transformer.attn_processors.keys():
        # 动态获取 hidden_dim (Qwen/Flux 通常 hidden_size)
        #transformer.config['num_attention_heads'] * self.transformer.config['attention_head_dim']
        style_hidden_dim = flux_transformer.config['num_attention_heads'] * flux_transformer.config['attention_head_dim']
        #print(f"Config hidden dim for layer {name}: {style_hidden_dim}")
        ...
        #print(f"Replacing processor for layer: {name} with style control processor.")
        attn_processors[name] = processor

    # 将新的 Processor 注入到模型中
    flux_transformer.set_attn_processor(attn_processors)
    #flux_transformer.to(accelerator.device) # 确保新层在正确的设备上

    # 4. 【核心修改】设置梯度：冻结全模型，只解冻 style kv proj
    flux_transformer.requires_grad_(False) # 先冻结所有
    flux_transformer.train()
    flux_transformer.enable_gradient_checkpointing() # 开启梯度检查点
    #flux_transformer.gradient_checkpointing_enable()

    trainable_params = []
    total_trainable_params = 0

    # 遍历模型，找到我们的自定义 Processor，解冻其中的两层
    ...
    print(f"Total trainable parameters (Style KV Projections): {total_trainable_params / 1e6:.4f} M")

    # 5. 优化器设置
    optimizer_cls = torch.optim.AdamW
    if args.adam8bit:
        optimizer = bnb.optim.Adam8bit(trainable_params, # 这里传入我们收集的参数列表
            lr=args.learning_rate,
            betas=(args.adam_beta1, args.adam_beta2),)
    else:
        optimizer = optimizer_cls(
            trainable_params, # 这里传入我们收集的参数列表
            lr=args.learning_rate,
            betas=(args.adam_beta1, args.adam_beta2),
    ...
    train_dataloader = loader(
        ...
        img_cache_dir=img_cache_dir, # 存 GT latents 的目录
        control_cache_dir=control_cache_dir,               # 存 Control 字典的目录
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(flux_transformer):
                ...
                if args.precompute_text_embeddings:
                    ...
                    #print(f"style_image_latents shape: {attention_kwargs['style_image_latents'].shape}, image_latents shape: {image_latents.shape},L_noise: {attention_kwargs['noise_patches_length']}, style_start_idx: {attention_kwargs['style_start_idx']}, style_end_idx: {attention_kwargs['style_end_idx']}")
                    #style_image_latents=control_img['style_image_latents'].to(dtype=weight_dtype).to(accelerator.device)
                    #L_noise=control_img['L_noise']
                    #style_start_idx=control_img['style_start_idx']
                    #style_end_idx=control_img['style_end_idx']
                    #control_img = control_img.to(dtype=weight_dtype).to(accelerator.device)
                    
                else:
                    print('Not implemented 不能走这个分支，代码没改')
                    img, prompts = batch
                with torch.no_grad():
                    ...
                    control_img = image_latents
                    #print(f"pixel_latents shape: {pixel_latents.shape}, control_img shape: {control_img.shape}")
                    latents_mean = (
                ...
                # packed_control_img = QwenImageEditPlusPipelineWithStyleControl._pack_latents(
                #     control_img,
                #     bsz, 
                #     control_img.shape[2],
                #     control_img.shape[3],
                #     control_img.shape[4],
                # )
                #print(f"packed_noisy_model_input shape: {packed_noisy_model_input.shape}, packed_control_img shape: {control_img.shape}")
                ...
                ##这里假设目标图和内容图条件图都是1024大小，所以位置编码直接这样硬编码了
                #print(f"packed_noisy_model_input shape: {packed_noisy_model_input.shape}, control_img shape: {control_img.shape}")
                packed_noisy_model_input_concated = torch.cat([packed_noisy_model_input, control_img], dim=1)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    # 2. 获取 unwrapped model
                    unwrapped_model = accelerator.unwrap_model(flux_transformer)
                    
                    # 3. 获取 state_dict (ZeRO-3 特有)
                    # 必须使用 accelerator.get_state_dict 来处理分片
                    # 这会把所有参数 gather 到 CPU 或主进程
                    full_state_dict = accelerator.get_state_dict(flux_transformer)
                    
                    # 4. 筛选 trainable 参数 (只在主进程做)
                    if accelerator.is_main_process:
                        ...
                        file_path = os.path.join(save_path, "style_control_layers.safetensors")
                        save_file(trainable_state_dict, file_path)
                        logger.info(f"Saved trainable style weights to {file_path}")
                    
                    # 6. 释放显存 (可选，防止 gather 后 OOM)
                    del full_state_dict
                    torch.cuda.empty_cache()
