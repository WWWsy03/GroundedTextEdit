...
# 导入你刚创建的 Processor
from grounded_processor_rope import GroundedQwenAttnProcessor
...
class MyGroundedQwenPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        print("🔥 [MyGroundedPipeline v12] 开始热插拔 Attention Processors...")
        
        if hasattr(self, "transformer") and hasattr(self.transformer, "transformer_blocks"):
            for i, block in enumerate(self.transformer.transformer_blocks):
                # v12 processor 不再需要 __init__ 参数
                block.attn.processor = GroundedQwenAttnProcessor()
            print(f"✅ [MyGroundedPipeline v12] {len(self.transformer.transformer_blocks)} 个 blocks 热插拔完成！")
        else:
            print("⚠️ [MyGroundedPipeline v12] 警告: 找不到 transformer.transformer_blocks。热插拔失败。")
        ...
        if images is not None:
            ...
            for image in images:
                ...
                if image.shape[1] != self.latent_channels:
                    ...
                    print(f"Encoded image to latents with shape: {image_latents.shape}")
                else:
                    image_latents = image
    ...
    def __call__(
        self,
        # 关键: `image` 参数现在是一个列表: [orig_image, control_image]
        ...
        # v14: 还原 erase_mask 检查
        if erase_mask is None:
            raise ValueError("`erase_mask` (PIL Image) 是必须的")
        
        orig_image = image[0]
        control_image = image[1]

        # --- 2. 复制原 __call__ 的所有检查和设置 ---
        ...
        # --- 3. 图像预处理 (v11/v12/v14 逻辑: 强制 H/W 相同) ---
        
        # 3.1 VAE 图像 (用于 Latent)
        vae_image_sizes = []
        vae_images = []
        for img in [orig_image, control_image]: # 分别处理
            image_width, image_height = img.size
            
            # --- v11/v12/v14: 强制 vae_images 使用与 noise 相同的尺寸 ---
        ...
        condition_images.append(self.image_processor.resize(orig_image, condition_height, condition_width))
        ...
        # --- v11/v12/v14: 这里的 shape 现在都相同了 (H_noise, W_noise) ---
        img_shapes = [
            [
                (1, height // self.vae_scale_factor // 2, width // self.vae_scale_factor // 2), # Noise
                (1, vae_image_sizes[0][1] // self.vae_scale_factor // 2, vae_image_sizes[0][0] // self.vae_scale_factor // 2), # Orig (now == Noise)
                (1, vae_image_sizes[1][1] // self.vae_scale_factor // 2, vae_image_sizes[1][0] // self.vae_scale_factor // 2), # Control (now == Noise)
            ]
        ] * batch_size
        ...
        latent_mask = F.interpolate(
            ...
            mode='bilinear', # <-- v14: 使用双线性插值
            align_corners=False
        )
        # latent_mask = (latent_mask > 0.5).float() # <-- v14: 移除二值化
        latent_mask = latent_mask.flatten(1) # (B, L_noise)

        # 8.3. 注入到 `self._attention_kwargs`
        ...
        # --- 9. Denoising loop (v14: 还原 v10 的时间步控制) ---
            ...
            for i, t in enumerate(timesteps):
                ...
                with self.transformer.cache_context("cond"):
                    noise_pred = self.transformer(
                        ...
                        img_shapes=img_shapes, # <-- v14: 共享 shape
                        txt_seq_lens=txt_seq_lens,
                        attention_kwargs=self.attention_kwargs, # <-- v14
                        return_dict=False,
                    )[0]
                ...
                if do_true_cfg:
                    with self.transformer.cache_context("uncond"):
                        neg_noise_pred = self.transformer(
                            ...
                            attention_kwargs=self._attention_kwargs, # <-- v14
                            return_dict=False,
                        )[0]
