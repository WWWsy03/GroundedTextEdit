    ...
    if args.precompute_text_embeddings:
        with torch.no_grad():
            ...
            for img_name in tqdm([i for i in os.listdir(args.data_config.control_dir) if ".png" in i or '.jpg' in i]):
                ...
                content_img = Image.open(content_path).convert('RGB').resize((origin_img.size))
                style_img= Image.open(style_img_path).convert('RGB').resize((origin_img.size))
                
                
                calculated_width, calculated_height, _ = calculate_dimensions(CONDITION_IMAGE_SIZE, content_img.size[0] / content_img.size[1])
    ...
    if args.precompute_image_embeddings:
        ...
        with torch.no_grad():
            for img_name in tqdm([i for i in os.listdir(args.data_config.control_dir) if ".png" in i or ".jpg" in i]):
                ...
                origin_img = Image.open(os.path.join(args.data_config.origin_dir, origin_img_name)).convert('RGB')
                content_img = Image.open(os.path.join(args.data_config.control_dir, img_name)).convert('RGB').resize((origin_img.size))
                print(f"content{content_img.size}")
                calculated_width, calculated_height, _ = calculate_dimensions(1024 * 1024, origin_img.size[0] / origin_img.size[1])
                style_img_name = img_name.replace('content', 'style')
                style_img = Image.open(os.path.join(args.data_config.style_dir, style_img_name)).convert('RGB').resize((origin_img.size))
                mask_img_name = img_name.replace('content', 'mask')
                mask_img_name=mask_img_name.replace('jpg','png')
                mask_img = Image.open(os.path.join(args.data_config.mask_dir, mask_img_name)).convert('RGB').resize((origin_img.size))
                ...
                content_img = text_encoding_pipeline.image_processor.preprocess(content_img, calculated_height, calculated_width).unsqueeze(2)
                ...
                print(f"imglatentsshape{image_latents.shape}")
                ...
                grid_h = calculated_height // vae_scale_factor // 2
                grid_w = calculated_width // vae_scale_factor // 2
                ...
                print(f"masktentor{mask_tensor.shape}")
